...
func _process(delta):
    ...
    if not target:
        return
    $OrientationLine.points[1] = orientation * 100
    var angle = orientation.angle_to(target.get_global_position() - position)
    var part = (angle / 0.1) * delta * speed
    orientation = orientation.rotated(part).normalized()
    # Should not rotate entire Node
    rotate(part)
